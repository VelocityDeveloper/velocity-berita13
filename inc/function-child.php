...
function velocity_posts($cat_id = null) {
    ...
    if(!empty($velocity_posts)){
        ...
        echo '<div class="row">';
            echo '<div class="col-md-5 text-muted mb-3 mb-md-0">';
            foreach(array_slice($velocity_posts, 0,1) as $post) {
                $post_id = $post->ID;
                echo '<div class="h-100">';
                echo '<div class="position-relative">';
                	echo do_shortcode('[resize-thumbnail width="410" height="300" linked="true" class="w-100" post_id="'.$post_id.'"]');
				echo '</div>';
                echo '<div class="pt-2">';
					echo '<small class="text-success">';
						velocity_post_date($post_id);
					echo '</small>';
					echo '<a class="fs-6 fw-bold text-color-theme lh-sm d-block mb-2" href="'.get_the_permalink($post_id).'">'.get_the_title($post_id).'</a>';
					echo do_shortcode('[velocity-excerpt count="100" post_id="'.$post_id.'"]');
				echo '</div>';
				echo '</div>';
            }
            echo '</div>';
            echo '<div class="col-md-7 ps-md-0">';
                foreach(array_slice($velocity_posts, 1,4) as $post) { 
					...
					echo '<div class="row mb-2">';
						...
						echo '</div>';
						echo '<div class="col-8 col-md-9 ps-md-2">';
							echo '<small class="d-block text-success mb-1">';
								echo velocity_post_date($post_id);
							echo '</small>';
							echo '<div class="mb-1">';
